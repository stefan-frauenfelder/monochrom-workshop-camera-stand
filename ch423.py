...
class Ch423:
    ...
    def enable_interrupt(self):
        ...
        value = self.gpio_digital_read(self.eGPIO_TOTAL)
        self.gpio_digital_write(self.eGPIO_TOTAL, value)
    ...
    def poll_interrupts(self):
        ...
        while i < 8:
            ...
            if self._cbs[i] != 0 and bit != bit1:
                if (((self._mode[i] == self.eHIGH) or (self._mode[i] == self.eRISING)) and (bit != 1)) or (
                        (self._mode[i] == self.eLOW or self._mode[i] == self.eFALLING) and (bit != 0)):
                    i += 1
                    continue
